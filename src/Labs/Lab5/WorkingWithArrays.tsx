    ...
    return (
      <div id="wd-working-with-arrays">
        ...
        <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
            ...
            <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
                onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
            <input type="checkbox" className="form-check-input" checked={todo.completed} onChange={(e) => setTodo({ ...todo, completed: e.target.checked })} id="completedCheck"/>
            <label htmlFor="completedCheck" className="form-check-label ml-2">Completed</label>
        <br /><br /><hr />
